...
struct GridSpec {
    ...
    double curv_radius = std::numeric_limits<double>::infinity();  // inf = flat
    bool   curv_concave_up = true;
    bool   curv_normal_align = true;  // true = bores perpendicular to surface, false = axial
};
...
static bool parse_curvature_token(const std::string &tok,
                                  double &radius,
                                  bool &concave_up,
                                  bool &normal_align) {
    std::vector<std::string> parts;
    ...
    if (parts[0] == "inf" || parts[0] == "INF" || parts[0] == "Inf") {
        radius = std::numeric_limits<double>::infinity();
    } else {
        if (!parse_double_str(parts[0], radius) || !(radius > 0.0)) return false;
    }
    if (!parse_bool_token(parts[1], concave_up)) return false;
    // Third part optional; default = normal-aligned (1)
    if (parts.size() >= 3) {
        return parse_bool_token(parts[2], normal_align);
    }
    normal_align = true;
    return true;
}
...
// Axial x-position of the curved upstream surface at radial position y.
// For a concave-upstream sphere (vertex at grid.x0): x increases with |y|.
// For a convex-upstream sphere (vertex at grid.x0 + thickness): x decreases with |y|.
// Returns grid.x0 for flat grids (curv_radius == inf).
static inline double curved_surface_x(const GridSpec &grid, double y) {
    const double R = grid.curv_radius;
    if (!std::isfinite(R)) return grid.x0;
    const double ry = y / R;
    if (std::fabs(ry) >= 1.0) return grid.x0;   // clamp: outside sphere domain
    const double dz = R * (1.0 - std::sqrt(1.0 - ry * ry));
    return grid.curv_concave_up ? (grid.x0 + dz) : (grid.x0 - dz);
}

// True if the 2-D point (x, y) lies within any aperture bore of the (curved) grid.
//
// aperture_alignment options (grid.curv_normal_align):
//   true  (normal)  — bore axis is perpendicular to the curved surface.
//                     Check: perpendicular distance from tilted bore axis < r_ap.
//   false (axial)   — bore axis is parallel to the beam axis (x axis).
//                     Check: |y - y_ap| < r_ap  (same as flat grid).
static inline bool in_any_aperture_curved(double x, double y,
                                          const GridSpec &grid,
                                          double delta) {
    const double R = grid.curv_radius;
    const size_t n = std::min(grid.offs.size(), grid.rads.size());
    for (size_t i = 0; i < n; ++i) {
        const double y_ap = grid.offs[i];
        const double r_ap = grid.rads[i] + delta;
        // Flat grid or axial-aligned bores: simple radial (Y) distance check
        if (!std::isfinite(R) || !grid.curv_normal_align) {
            if (std::fabs(y - y_ap) < r_ap) return true;
            continue;
        }
        // Normal-aligned bores: perpendicular distance from the tilted bore axis.
        // Aperture tilt: sin(alpha) = y_ap / R, cos(alpha) = sqrt(1 - sin^2)
        const double sin_a = std::clamp(y_ap / R, -1.0, 1.0);
        const double cos_a = std::sqrt(1.0 - sin_a * sin_a);
        // Aperture centre on curved surface
        const double dz   = R * (1.0 - cos_a);
        const double x_ap = grid.curv_concave_up ? (grid.x0 + dz) : (grid.x0 - dz);
        // Perpendicular direction to bore axis (cos_a, -sin_a) is (sin_a, cos_a)
        const double perp = std::fabs((x - x_ap) * sin_a + (y - y_ap) * cos_a);
        if (perp < r_ap) return true;
    }
    return false;
}
...
static inline bool grid_fn_idx(size_t idx, double x, double y) {
    ...
    const double x_surf = curved_surface_x(grid, y);
    if (x >= x_surf && x <= x_surf + grid.thickness) {
        const double delta = grid_delta(grid, x);
        return !in_any_aperture_curved(x, y, grid, delta);
    }
    return false;
}
...
auto sec_since = [](const SteadyClock::time_point &a, const SteadyClock::time_point &b) -> double {
    ...
    if (using_grid_stack) {
        ...
        for (size_t gi = 0; gi < grid_tokens.size(); ++gi) {
            ...
            if (fields.size() < 5 || fields.size() > 9) {
                std::fprintf(stderr, "ERROR: GRID_STACK entry %zu must have 5-9 fields\n", gi);
                return 2;
            }
            ...
            if (fields.size() >= 9 &&
                !parse_curvature_token(fields[8], grid.curv_radius,
                                       grid.curv_concave_up, grid.curv_normal_align)) {
                std::fprintf(stderr, "ERROR: GRID_STACK entry %zu has invalid curvature\n", gi);
                return 2;
            }
    ...
    // meta.json for sweep bookkeeping
    {
        ...
        for (size_t gi = 0; gi < g_grids.size(); ++gi) {
            ...
            m << "]";
            // Spherical curvature parameters
            if (std::isfinite(grid.curv_radius)) {
                m << ", \"curv_radius_m\":" << std::setprecision(10) << grid.curv_radius;
            } else {
                m << ", \"curv_radius_m\":null";
            }
            m << ", \"curv_concave_up\":"   << (grid.curv_concave_up   ? "true" : "false");
            m << ", \"curv_normal_align\":" << (grid.curv_normal_align ? "true" : "false");
            // Per-aperture tilt angle (degrees) — alpha = asin(|r| / R)
            m << ", \"tilt_deg\":[";
            for (size_t i = 0; i < grid.offs.size(); ++i) {
                double r = std::fabs(grid.offs[i]);
                double tilt = 0.0;
                if (std::isfinite(grid.curv_radius) && grid.curv_radius > 0.0
                        && r < grid.curv_radius)
                    tilt = std::asin(r / grid.curv_radius) * (180.0 / M_PI);
                m << std::setprecision(6) << tilt;
                if (i + 1 < grid.offs.size()) m << ", ";
            }
            m << "]";
            // Per-aperture sagitta — axial displacement of aperture centre from grid vertex
            m << ", \"sagitta_m\":[";
            for (size_t i = 0; i < grid.offs.size(); ++i) {
                double r = std::fabs(grid.offs[i]);
                double sag = 0.0;
                if (std::isfinite(grid.curv_radius) && grid.curv_radius > 0.0
                        && r < grid.curv_radius) {
                    double ry = r / grid.curv_radius;
                    sag = grid.curv_radius * (1.0 - std::sqrt(1.0 - ry * ry));
                }
                m << std::setprecision(10) << sag;
                if (i + 1 < grid.offs.size()) m << ", ";
            }
            m << "]}";
    ...
    if (RUN_SOLVE) {
        ...
        if (!ENABLE_IONS) {
            ...
            // Particle count per aperture
            const int n_ap_count = std::max(1, (int)g_scr_offs.size());
            const int n_part_per = std::max(1, ION_NPART / n_ap_count);

            // Injection energy: potential drop between plasma and accel side
            const double E0_ev  = std::max(1.0, std::fabs(PLASMA_UP_V - VA_V));

            // Screen grid curvature parameters for tilted Bohm injection
            const double scr_R          = g_grids.empty() ? std::numeric_limits<double>::infinity()
                                                           : g_grids[0].curv_radius;
            const bool   scr_concave_up = g_grids.empty() ? true : g_grids[0].curv_concave_up;
            const double scr_x0         = g_grids.empty() ? xs0  : g_grids[0].x0;

            std::printf("ion iteration: mesh=%dx%d  n_ap=%zu  n_part_per=%d  iter_max=%d\n",
                        (int)scharge.size(0), (int)scharge.size(1),
                        g_scr_offs.size(), n_part_per, ION_ITER_MAX);
            std::fflush(stdout);
            ...
            for (int it = 0; it < ION_ITER_MAX; ++it) {
                ...
                std::printf("  iter %d/%d: injecting + tracing particles...\n", it+1, ION_ITER_MAX);
                std::fflush(stdout);
                pdb.clear();
                ...
                for (size_t ai = 0; ai < n_ap; ++ai) {
                    ...
                    // Per-aperture curved emission geometry.
                    // For flat grids (scr_R == inf): sin_a=0, cos_a=1, dz=0 →
                    //   identical to old vertical line at xs0 - 2h.
                    double sin_a = 0.0, cos_a = 1.0, x_ap_center = scr_x0;
                    if (std::isfinite(scr_R) && scr_R > 0.0) {
                        sin_a = std::clamp(off / scr_R, -1.0, 1.0);
                        cos_a = std::sqrt(1.0 - sin_a * sin_a);
                        const double dz = scr_R * (1.0 - cos_a);
                        x_ap_center = scr_concave_up ? (scr_x0 + dz) : (scr_x0 - dz);
                    }

                    // Emission line centre: 2h upstream along outward surface normal (-cos_a, sin_a)
                    const double x_emit_ap = std::max(xmin + 2.0*h, x_ap_center - 2.0*h * cos_a);
                    const double y_emit_ap = off + 2.0*h * sin_a;

                    // Emission line endpoints: ±rad along surface tangent (sin_a, cos_a).
                    // IBSimu injects particles perpendicular to this line, i.e., along the
                    // aperture normal (cos_a, -sin_a) — the Bohm-velocity direction.
                    const double x_lo = x_emit_ap - rad * sin_a;
                    const double y_lo = y_emit_ap - rad * cos_a;
                    const double x_hi = x_emit_ap + rad * sin_a;
                    const double y_hi = y_emit_ap + rad * cos_a;
                    ...
                    pdb.add_2d_beam_with_energy(
                        ...
                        x_lo, y_lo,    // tilted emission line start
                        x_hi, y_hi     // tilted emission line end
                    );
                }
                pdb.iterate_trajectories(scharge, efield, bfield);
                std::printf("  iter %d/%d: trajectories done  npart=%u\n",
                            it+1, ION_ITER_MAX, (unsigned)pdb.size());
                std::fflush(stdout);
            ...
            // For curved grids the upstream/downstream faces are not flat; the on-axis
            // vertex is at xs0/xs1/xa0/xa1, but off-axis apertures are displaced axially
            // by up to max_sagitta.  We must place the diagnostic plane beyond the
            // outermost aperture exit so that every beam particle crosses it exactly once.
            // For flat grids max_sagitta==0 and the positions collapse to the
            // conventional xs1+0.5h / xa1+0.5h.
            auto max_sagitta_of = [](const GridSpec &g) -> double {
                if (!std::isfinite(g.curv_radius) || g.curv_radius <= 0.0 || g.max_radius <= 0.0)
                    return 0.0;
                const double ry = std::min(1.0, g.max_radius / g.curv_radius);
                return g.curv_radius * (1.0 - std::sqrt(1.0 - ry * ry));
            };
            const double sag_scr = max_sagitta_of(first_grid);
            const double sag_acc = max_sagitta_of(last_grid);

            // Probe planes (offset by +0.5*h so we sample *in vacuum*, not on an electrode surface)
            // PG: just downstream of outermost screen aperture exit (still in inter-grid gap).
            // AG: just downstream of outermost accel aperture exit.
            const double x_pg_plane = xs1 + sag_scr + 0.5*h;
            const double x_ag_plane = xa1 + sag_acc + 0.5*h;
            ...
            // ---- 2.2b Per-beamlet current at AG-exit plane ----
            // Center beamlet = aperture with smallest |screen offset|.
            // Outer beamlet  = aperture with largest  |screen offset|.
            // Window = ±half the minimum spacing between adjacent sorted offsets
            // (falls back to 2×aperture-radius when only one beamlet).
            double I_center_beamlet_Apm  = 0.0;
            double I_outer_beamlet_Apm   = 0.0;
            double y_center_beamlet_m    = 0.0;
            double y_outer_beamlet_m     = 0.0;
            double beamlet_half_window_m = 0.0;
            int    n_beamlets_layout     = (int)g_scr_offs.size();

            if (n_beamlets_layout > 0) {
                std::vector<double> soffs = g_scr_offs;
                std::sort(soffs.begin(), soffs.end());
                double min_sp = std::numeric_limits<double>::max();
                for (size_t i = 1; i < soffs.size(); ++i) {
                    double sp = soffs[i] - soffs[i-1];
                    if (sp > 0.0 && sp < min_sp) min_sp = sp;
                }
                beamlet_half_window_m = (soffs.size() == 1 || min_sp == std::numeric_limits<double>::max())
                    ? (2.0 * g_a_scr) : (0.5 * min_sp);

                // Center beamlet (min |offset|)
                {
                    int idx = 0; double best = std::fabs(g_scr_offs[0]);
                    for (int i = 1; i < n_beamlets_layout; ++i) {
                        double ab = std::fabs(g_scr_offs[i]);
                        if (ab < best) { best = ab; idx = i; }
                    }
                    y_center_beamlet_m = g_scr_offs[idx];
                }
                // Outer beamlet (max |offset|)
                {
                    int idx = 0; double best = std::fabs(g_scr_offs[0]);
                    for (int i = 1; i < n_beamlets_layout; ++i) {
                        double ab = std::fabs(g_scr_offs[i]);
                    ...
                    y_outer_beamlet_m = g_scr_offs[idx];
                }

                double Itmp = 0.0, ym = 0.0, yr = 0.0, yrc = 0.0, yam = 0.0, yamc = 0.0;
                current_and_width_at_plane_window(
                    x_ag_plane,
                    y_center_beamlet_m - beamlet_half_window_m,
                    y_center_beamlet_m + beamlet_half_window_m,
                    Itmp, ym, yr, yrc, yam, yamc);
                I_center_beamlet_Apm = Itmp;

                Itmp = ym = yr = yrc = yam = yamc = 0.0;
                current_and_width_at_plane_window(
                    x_ag_plane,
                    y_outer_beamlet_m - beamlet_half_window_m,
                    y_outer_beamlet_m + beamlet_half_window_m,
                    Itmp, ym, yr, yrc, yam, yamc);
                I_outer_beamlet_Apm = Itmp;
            }
...
const double P_sys_norm_sm = P_sys_norm_ag;
                ...
                mj << "    \"center_beamlet\": {\n";
                mj << "      \"screen_offset_m\": " << std::setprecision(10) << y_center_beamlet_m   << ",\n";
                mj << "      \"I_Apm\": "           << std::setprecision(10) << I_center_beamlet_Apm << ",\n";
                mj << "      \"I_A\": "             << std::setprecision(10) << I_center_beamlet_A   << "\n";
                mj << "    },\n";
                mj << "    \"outer_beamlet\": {\n";
        ...
        std::printf("solve complete — writing outputs...\n");
        std::fflush(stdout);

        if (writepng) {
            std::printf("writing geometry PNG: %s\n", png.c_str());
            std::fflush(stdout);
            // Full view
	    ...
	    gp.set_font_size(FONT_PX);
            ...
            // Helper: x position of an aperture at radial offset |r_ap| on a given grid.
            // For curved grids this is the vertex x0 + sagitta(r_ap).
            auto ap_x_on_grid = [](const GridSpec &gr, double r_ap) -> double {
                if (!std::isfinite(gr.curv_radius) || gr.curv_radius <= 0.0) {
                    return gr.x0;
                }
                double ry = std::clamp(std::fabs(r_ap) / gr.curv_radius, 0.0, 1.0);
                double dz = gr.curv_radius * (1.0 - std::sqrt(1.0 - ry * ry));
                return gr.curv_concave_up ? (gr.x0 + dz) : (gr.x0 - dz);
            };

            // Per-aperture close-up: x-range is anchored to the aperture's curved
            // position rather than always spanning from xs0 to xa1.
            // plot_closeup: center view on the midpoint between the screen and
            // accel aperture centres for this beamlet pair.
            // r_scr: radial offset of screen aperture (y-coord on screen surface)
            // r_acc: radial offset of accel aperture  (y-coord on accel surface)
            // Each grid uses its OWN r value so the sagitta is computed correctly
            // even when screen and accel radii of curvature differ (concentric design).
            auto plot_closeup = [&](double r_scr, double r_acc,
                                    const std::string &outpath) {
                const auto &first_g = g_grids.front();
                const auto &last_g  = g_grids.back();

                // x: midpoint between screen upstream face and accel downstream face
                const double x_ap_first = ap_x_on_grid(first_g, r_scr);
                const double x_ap_last  = ap_x_on_grid(last_g,  r_acc) + last_g.thickness;
                const double xlo = x_ap_first - pad;
                const double xhi = x_ap_last  + pad;

                // y: midpoint between the two aperture centre y-positions
                const double y_center = 0.5 * (r_scr + r_acc);
                // half-span: enough to show both aperture centres plus bore margin (+20%)
                const double y_half_gap = 0.5 * std::fabs(r_scr - r_acc);
                double ymax_local = (y_half_gap + a_max + max_delta2 + pad) * 1.20;
                ...
                const double xr = xhi - xlo, yr = y1 - y0;
                const int   H = BASE;
                ...
                gpc.set_ranges(xlo, y0, xhi, y1);
                gpc.set_font_size(FONT_PX);
                gpc.set_size(W, H);
                gpc.plot_png(outpath.c_str());
            };

            // Build the list of unique aperture pairs (positive y only, deduplicated).
            // Store (screen_offset, accel_offset) so we can compute the curved x-center.
            ...
            for (size_t ci = 0; ci < close_pairs.size(); ++ci) {
                // Use each grid's own aperture y-offset so sagitta is computed
                // against the correct radius of curvature for each grid.
                const double r_scr = std::fabs(close_pairs[ci].first);
                const double r_acc = std::fabs(close_pairs[ci].second);
                std::string close = add_suffix_before_ext(png, "_closeup");
                if (ci > 0) {
                    close = add_suffix_before_ext(png, "_closeup_p" + std::to_string(ci));
                }
                plot_closeup(r_scr, r_acc, close);
	...
	gp.set_font_size(FONT_PX);
        ...
        // Geometry-only close-up: same curved x-centering as the solved path.
        auto ap_x_on_grid_geom = [](const GridSpec &gr, double r_ap) -> double {
            if (!std::isfinite(gr.curv_radius) || gr.curv_radius <= 0.0) return gr.x0;
            double ry = std::clamp(std::fabs(r_ap) / gr.curv_radius, 0.0, 1.0);
            double dz = gr.curv_radius * (1.0 - std::sqrt(1.0 - ry * ry));
            return gr.curv_concave_up ? (gr.x0 + dz) : (gr.x0 - dz);
        };

        auto plot_closeup = [&](double r_scr, double r_acc, const std::string &outpath) {
            const auto &first_g = g_grids.front();
            const auto &last_g  = g_grids.back();
            const double xlo = ap_x_on_grid_geom(first_g, r_scr) - pad;
            const double xhi = ap_x_on_grid_geom(last_g,  r_acc) + last_g.thickness + pad;

            const double y_center  = 0.5 * (r_scr + r_acc);
            const double y_half_gap = 0.5 * std::fabs(r_scr - r_acc);
            double ymax_local = (y_half_gap + a_max + max_delta + pad) * 1.15;
            double y0 = y_center - ymax_local, y1 = y_center + ymax_local;
